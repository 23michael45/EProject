...
static void ModifyTexturePixels()
{
	...
	int textureRowPitch;
	void* textureDataPtr = s_CurrentAPI->BeginModifyTexture(textureHandle, width, height, 4, &textureRowPitch);
	if (!textureDataPtr)
		return;
	...
	s_CurrentAPI->EndModifyTexture(textureHandle, width, height, textureRowPitch,4, textureDataPtr);
}
...
extern "C" {
	...
	{
		dataPtr = s_CurrentAPI->BeginModifyTexture(textureHandle, textureWidth, textureHeight, channel, outRowPitch);
	}
	...
	{
		s_CurrentAPI->EndModifyTexture(textureHandle, textureWidth, textureHeight, channel, rowPitch, dataPtr);
	}
	...
	{
		dataPtr = s_CurrentAPI->BeginModifyVertexBuffer(bufferHandle, outBufferSize);
	}
	...
	{
		s_CurrentAPI->GetTextureBuffer(bufferHandle, textureWidth,textureHeight,channel, dataPtr);
	}
	void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API GetVertextBuffer(void* bufferHandle, void* &dataPtr, int& len)
	{
		s_CurrentAPI->GetVertextBuffer(bufferHandle, dataPtr, len);
	}
