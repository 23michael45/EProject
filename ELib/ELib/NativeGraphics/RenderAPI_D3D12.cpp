...
void* RenderAPI_D3D12::BeginModifyTexture(void* textureHandle, int textureWidth, int textureHeight, int channel, int* outRowPitch)
{
	...
	// Fill data
	const UINT64 kDataSize = textureWidth * textureHeight * channel;
	ID3D12Resource* upload = GetUploadResource(kDataSize);
	void* mapped = NULL;
	upload->Map(0, NULL, &mapped);
	*outRowPitch = textureWidth * channel;
	return mapped;
}


void RenderAPI_D3D12::EndModifyTexture(void* textureHandle, int textureWidth, int textureHeight, int channel, int rowPitch, void* dataPtr)
{
	ID3D12Device* device = s_D3D12->GetDevice();
...
void RenderAPI_D3D12::GetTextureBuffer(void* bufferHandle,int textureWidth, int textureHeight, int channel,void* dataPtr)
{

}
void RenderAPI_D3D12::GetVertextBuffer(void* bufferHandle, void* &dataPtr, int& len)
{
